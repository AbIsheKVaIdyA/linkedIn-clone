...
PostSchema.methods.likePost = async function (userId: string) {
  try {
    ...
    console.log("error when linking post", error);
  }
};
...
PostSchema.methods.unlikePost = async function (userId: string) {
  try {
    ...
    console.log("error when unlinking post", error);
  }
};
...
PostSchema.methods.getAllComments = async function (){
    try{
        await this.populate({
            path:"comments",
            options:{sort:{createdAt:-1}} // sort by date from new to old
          });
...
PostSchema.statics.getAllPosts = async function () {
    try{
        ...
        .sort( { createdAt : -1 } )// sort by date from new to old      
        .populate({
            path: 'comments',
            options:{sort:{createdAt:-1}},
         }).lean() //convert mongoose object into plain JS object
